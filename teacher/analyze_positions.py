...
def played_rank_in_pvs(uci_move: str, pvs: list) -> int:
    # If played move equals first token of PV (UCI), rank = its multipv index
    # Our PV tokens are UCI strings in helpers.
    for d in pvs:
        pv = d.get("pv", [])
        if pv and pv[0].lower().startswith(uci_move[:4]):  # compare squares; promotion tolerant
            return d["multipv"]
    return 99
...
def main():
    ...
    for row in tqdm(df.itertuples(index=False), total=len(df), desc="Analyzing"):
        ...
        if not post:
            continue
        post_best_cp = cp_from_score(post[0]["score"], 'w' if board.turn else 'b')  # perspective flips

        # cp_loss for the mover (negative = better, positive = worse)
        # We compare "value for mover" before vs after; since cp_from_score is from side_to_move,
        # we used 'side' for pre, and after pushing move, side_to_move flips, so invert sign.
        cp_loss = pre_best_cp - (-post_best_cp)

        played_rank = played_rank_in_pvs(played, pre)
        # simple material delta (optional): here we don't compute, set 0; you can enhance later
        material_delta_cp = 0

        motifs = detect_motifs(material_delta_cp=material_delta_cp, pre_top_gap=pre_gap)
        robust = True  # placeholder: later re-check at deeper depth for suspected brilliants
